...
public class SendMessage : MonoBehaviour {
    ...
    //create new object to be location of text
    public GameObject textLocation;
    [SerializeField] private BlockListManager blockList;
    ...
    void Start()
    {
        ...
        pubnub.SubscribeCallback += (sender, e) =>
        {
            ...
            if (message.MessageResult != null)
            {
                ...
                string playerUsername = chatmessage.username.Remove(chatmessage.username.Length - 2, 1);

                Debug.Log(blockList.playerNames.Count);

                if (blockList.playerNames.Count != 0)
                {

                    foreach (var item in blockList.playerNames)
                    {
                        Debug.Log(playerUsername);
                        Debug.Log(item);
                        if (playerUsername.Trim() == item.Trim())
                        {
                            //niet laten zien
                            Debug.Log("true");
                        }
        ...
        //pubnub.DeleteMessages().Channel(channel).Start(15526611838554310).End(15526611838554309);

        //for (int i = 0; i < 5; i++)
        //{
        //    JSONInformation emptyMessage = new JSONInformation();
        //    emptyMessage.text = "";
        //    emptyMessage.username = "";

        //    CreateChat(emptyMessage);
        //}
    }



    void FilterBlockList()
    {
    ...
    void CreateChat(JSONInformation payLoad){
        ...
        chatMessage.transform.position = new Vector3(textLocation.transform.position.x - paddingX, textLocation.transform.position.y - paddingY + padding - (indexcounter * height), 1F);
    ...
    void SyncChat() {
        ...
        if (indexcounter > maxMessagesToDisplay)
        {
            ...
            foreach (GameObject moveChat in chatMessageQueue)
            {
                ...
                moveText.position = new Vector3(textLocation.transform.position.x - paddingX, textLocation.transform.position.y - paddingY + padding - (c * height), 1F);
                c++;
            }
        }
    }
    ...
    void TaskOnClick()
    {
        ...
        publishMessage.username = string.Concat(playerNameText.text, ": ");
